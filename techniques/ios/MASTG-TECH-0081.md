...
The `lsof` command, when invoked with option `-i`, gives the list of open network ports for all active processes on the device. To get a list of open network ports for a specific process, the `lsof -i -a -p <pid>` command can be used, where the `-a` (AND) option is used for filtering. Below is a filtered output for PID 1.
