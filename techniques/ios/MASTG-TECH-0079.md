...
The _provisioning profile_ is a plist file signed by Apple that adds your code-signing certificate to the list of accepted certificates on one or more devices. In other words, this explicitly allows your app to run for specific reasons, such as debugging on selected devices (development profile). The provisioning profile also includes the _entitlements_ granted to your app. The _certificate_ contains the private key you'll use to sign.

A valid provisioning profile can only be obtained from Apple. This means that you need a valid Apple account.

!!! info

    You can obtain a valid provisioning profile for both regular Apple accounts and Apple Developer accounts. There are two essential differences between the two types:
    ...
    **Wildcard certificates**
    ...
    An Apple Developer account costs $99 per year and is a nice-to-have due to its longer expiration, but it is not required.

The steps below work for both a regular Apple account and an Apple Developer account, but do require a macOS host.
...
This file can be copied to your local directory and used to sign any IPA file, including those with a different identifier.
