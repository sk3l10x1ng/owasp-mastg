...
`lsof` is a powerful command that provides a wealth of information about running processes. It can provide a list of all open files, including streams, network files, and regular files. When invoking the `lsof` command without any option, it will list all open files belonging to all active processes on the system, while when invoking with the flags `-c <process name>` or `-p <pid>`, it returns the list of open files for the specified process. The [man page](http://man7.org/linux/man-pages/man8/lsof.8.html "Man Page of lsof") details additional options.
