...
- Searching for occurrences of specific data.
- After searching, get the location of a particular offset in the memory map.
...
While you're searching or exploring the app memory, you can always verify your current offset in the memory map. Instead of noting and searching for the memory address in this list, you can run `:dm`. You'll find an example in the section titled "In-Memory Search".
...
In-memory search is a valuable technique for detecting sensitive data in the app's memory.
...
For now, we'll keep the defaults and focus on string search. In this first example, you can start by searching for something that you know should be located in the main binary of the app (for example, the name of the @MASTG-APP-0028 app):
...
Now take the first hit, seek it, and check your current location in the memory map:
...
Now you know that the string is located in an rw- (read and write) region of the memory map.
...
In-memory search can help quickly determine whether specific data is located in the main app binary, a shared library, or another region. You may also use it to test the app's behavior regarding how data is stored in memory. For instance, you could continue the previous example by clicking Login and searching again for occurrences of the data. Also, you may check whether you can still find those strings in memory after login is complete to verify that this _sensitive data_ is wiped from memory after use.

## Memory Dump

You can dump the app's process memory with @MASTG-TOOL-0074 and @MASTG-TOOL-0106. To take advantage of these tools on a non-jailbroken device, the Android app must be repackaged with `frida-gadget.so` and re-signed. A detailed explanation of this process is described in @MASTG-TECH-0090. To use these tools on a jailbroken phone, ensure frida-server is installed and running.
...
Next, we can seek its address using `s 0x0036f800` or `s hit4_0` and print it using `psw` (which stands for _print string wide_) or use `px` to print its raw hexadecimal values:
