...
In @MASTG-TECH-0052, you can learn how to use @MASTG-TOOL-0055 to use SSH via USB. When performing dynamic analysis, it's helpful to use an SSH tunnel to route our traffic through Burp, which is running on our computer. Let's get started:
...
2. Connect to _any_ Wi-Fi (you can literally connect to any Wi-Fi, as the traffic for ports 80 and 443 will be routed through USB, as we are just using the Proxy Setting for the Wi-Fi, so we can set a global Proxy)
